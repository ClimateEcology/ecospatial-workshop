...
The table contains each CDL value land use code (lucode), it's corresponding class name and the values for several indices. 

```{r read in koh table}
reclass_table <- read.csv("data/cdl_reclass_koh.csv")
head(reclass_table)
names(reclass_table)[1] <- "Class_Names"
```

We will select just the columns corresponding to the land use code and the spring floral resources to reclassify our county CDL and generate a map of spring floral resources across the county
```{r reclass to forage}
county_floral_sp <- classify(county_cdl,reclass_table[,c("Class_Names","floral_resources_spring_index")])

plot(county_floral_sp)

values(county_cdl) <- as.character(values(county_cdl))
county_cdl_r <- classify(county_cdl,reclass_table[,c("lucode","CLASS_NAME")])
```
...
```
and look at at individual grid cell values. The 
```{r inspect values}
...
```{r check learning raster, class.source = 'fold-hide'}
