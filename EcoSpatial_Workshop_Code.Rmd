...
Right now we are viewing the raw numeric codes of the CDL. These codes mean something: the crop land cover class. We can tell `terra` what these codes mean so that it will show the land class names rather than the raw values. The information for these land cover values (along with their colors) are stored in the color table in `data/cdl_colormap.csv`.
...
The table contains each CDL value, its corresponding class name and the values for several indices. 
...
```{r}
writeRaster(county_cdl, "data/centre_county_cdl_2015.tif", overwrite=TRUE, datatype="INT1U")
writeRaster(county_floral_sp, "data/centre_county_springfloral_2015.tif", overwrite=TRUE)
```


## Check your learning

1. Use beeshiny to download the CDL raster for any county for any year you want 

2. Move the CDL raster to the /data folder in your ecospatial-workshop directory

3. Read the raster into R as a terra SpatRaster

4. Define CDL value codes with land cover class names and assign colors using `cdl_colormap`
...
Using `ggplot2` and `sf` we can add our points to our earlier map of Centre county 
